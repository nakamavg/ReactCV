...
const ProjectCard = ({ project, index }) => {
  ...
  return (
    ...
    >
      ...
      {project.partner && project.link && (
        <p className="text-yellow-300 mb-2">
          <strong>Colaboradores:</strong> {renderPartners(project.partner, project.link)}
        </p>
      )}
      ...
      {project.link && Array.isArray(project.link) && project.link.length > 0 && (
        <div className="text-white mb-4">
          <a
            href={project.link[0]?.trim()}
            target="_blank"
            rel="noopener noreferrer"
            className="text-yellow-300 hover:underline"
          >
